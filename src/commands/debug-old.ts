...
export const data = new SlashCommandBuilder()
  .setName("debug")
  .setDescription(
    "Debug commands for testing bot functionality (requires debug key)"
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName("full-test")
      .setDescription("Run full system test cycle with debug email")
      ...
      .addStringOption((option) =>
        option
          .setName("debug-email")
          .setDescription("Email to send test reports to (default: your email)")
          .setRequired(false)
      )
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName("test-email")
      .setDescription("Send a test email to verify SMTP configuration")
      .addStringOption((option) =>
        option.setName("key").setDescription("Debug key").setRequired(true)
      )
  )
  .addSubcommand((subcommand) =>
    subcommand
      .setName("monthly-header")
      .setDescription("Post a monthly header (normally runs on 1st of month)")
      .addStringOption((option) =>
        option.setName("key").setDescription("Debug key").setRequired(true)
      )
  )
  .addSubcommand((subcommand) =>
    subcommand
      ...
      .setDescription(
        "Generate and send monthly report (normally runs on 21st)"
      )
      .addStringOption((option) =>
        option.setName("key").setDescription("Debug key").setRequired(true)
      )
      .addStringOption((option) =>
        option
          .setName("start-date")
          .setDescription("Start date (YYYY-MM-DD, optional)")
          .setRequired(false)
      )
      .addStringOption((option) =>
        option
...
async function handleFullTest(
  ...
  const debugEmail =
    interaction.options.getString("debug-email") ||
  ...
  try {
    ...
    // Save original boss email
    const originalBossEmail = process.env.BOSS_EMAIL;

    // Temporarily set debug email
    ...
    try {
      ...
      if (channel && channel.isTextBased()) {
        await (channel as TextChannel).send(
          `🧪 Full system test completed by ${interaction.user.tag}\n${
            allPassed ? "✅ All systems operational" : "⚠️ Some tests failed"
          }\nTests sent to: ${debugEmail}`
        );
      }
    } catch (error) {
      console.error("Failed to post summary to channel:", error);
    }
  }
}
...
async function handleTestEmail(
  ...
  try {
    await sendTestEmail();
    await interaction.editReply({
      content: `✅ Test email sent successfully to **${process.env.BOSS_EMAIL}**\n\nCheck your inbox (and spam folder)!`,
    });
  } catch (error) {
    throw new Error(
...
async function handleMonthlyHeader(
  ...
  if (!channelId) {
    throw new Error("LOG_CHANNEL_ID not configured in environment variables");
  }
...
async function handleMonthlyReport(
  ...
  if (startDateInput && endDateInput) {
    ...
    // Use default billing period (22nd of last month to 21st of this month)
    const today = new Date();
  ...
  try {
    ...
    if (channelId) {
      const channel = await interaction.client.channels.fetch(channelId);
      if (channel && channel.isTextBased()) {
        const message = `✅ Monthly report for **${startDateString}** to **${endDateString}** has been generated and emailed to ${process.env.BOSS_EMAIL}`;
        await (channel as TextChannel).send(message);
      }
    }

    await interaction.editReply({
      content: `✅ Monthly report generated and sent!\n\n**Period:** ${startDateString} to ${endDateString}\n**Sent to:** ${process.env.BOSS_EMAIL}\n\nCheck your email inbox!`,
    });
  } catch (error) {
    throw new Error(
