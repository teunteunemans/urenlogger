...
import { MonthlyReport, UserHoursSummary, HourLog } from "@/lib/types";
 ...
 */
export function aggregateHoursByUser(logs: HourLog[]): UserHoursSummary[] {
 ...
 *
 * Note: Discord log channel notification is not included here (no discord.js client).
 * Phase 3 may add this via REST API if needed.
 */
export async function sendMonthlyReport(
  ...
  try {
    ...
    if (logs.length === 0) {
      console.log(
        "No hours logged for this billing period - sending empty report"
      );
    }
...
export async function sendTestEmail(): Promise<void> {
  try {
    ...
    if (logs.length === 0) {
      console.log(
        "No hours logged in current period - sending empty report"
      );
    } else {
