...
import { HourLog, User } from "@/lib/types";
 ...
 * Uses base64-decoded service account from env var (Vercel serverless can't mount files)
 */
async function initializeFirebaseInternal(): Promise<admin.firestore.Firestore> {
  ...
  if (!serviceAccountBase64) {
    throw new Error(
      ...
      "Set it to a base64-encoded Firebase service account JSON."
    );
  }
